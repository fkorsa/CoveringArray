...
void CA_Solution::allouerMemoire()
{
    ...
    occurencesCouples = Allocate4DArray(k, k, v, v, 0);
    sousContraintesSymboles = Allocate2DArray(k, v, 0);
    sousContraintesColonnes = Allocate1DArray(k, 0);
    deltas = Allocate3DArray(k, N, v, 0);
}

CA_Solution::CA_Solution(int val, int col, vector<int> resultat) :
    occurencesCouples(nullptr),
    sousContraintesColonnes(nullptr),
    sousContraintesSymboles(nullptr),
    deltas(nullptr)
{
	v = val;
	k = col;
...
CA_Solution::~CA_Solution()
{
    ...
    if(sousContraintesColonnes)
    {
        delete[] sousContraintesColonnes;
    }
    if(sousContraintesSymboles)
    {
        Delete2DArray(sousContraintesSymboles, k);
    }
    if(deltas)
    {
        Delete3DArray(deltas, k, N);
    }
    if(occurencesCouples)
    {
        Delete4DArray(occurencesCouples, k, k, v);
    }
}
...
void CA_Solution::mouvementSuivant()
{
    bool estFinal = false;
