...
vector<int> couvertureGlouton(int v, int k, float tolerance) // Glouton si tolerance = 0, complètement aléatoire si tolerance = 1
{
...
int main()
{
	/* Exemple de calcul, lecture, écriture de solution */
	...
	// Données utilisées pour la tournée de tests
	int v = 2;
	int k = 4;
	float tol = 0./3.;

	float facteur = 8.6/12; // facteur multiplicatif pour calculer le temps CPU sur la machine de référence
	int nbTests = 100;

	// Structures de récupération des données
	...
	// Calcul des extrema pour les coûts
	int min = couts[0];
	int max = couts[0];
	for (int i=0 ; i<nbTests ; i++) {
		if(couts[i] < min) {
	...
	// Calcul de l'écart type pour les coûts
	for (int i=0 ; i<nbTests ; i++)
	{
	...
	// Enregistrement dans un fichier de tous les coûts pour pouvoir les porter sur un graphique par la suite
